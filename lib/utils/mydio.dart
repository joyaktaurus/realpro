...
class MyDio {
  ...
  Future<dynamic> customPost(
    ...
    try {
      ...
      // Check for 302 status code and handle the redirect
      if (resp.statusCode == 302) {
        String? redirectUrl = resp.headers['location']?.first; // Get the redirect URL
        debugPrint('Redirecting to: $redirectUrl');
        // You can handle the redirect URL here, such as making another request or navigating the user.
        return redirectUrl; // Return the redirect URL or handle as necessary
      }

      // If the response status is 200, process the response data
      if (resp.statusCode == 200) {
        dynamic apiResp = jsonDecode(resp.data);
        return apiResp; // Return the API response
      }
      ...
      if (ex.type == DioErrorType.connectTimeout) {
        ...
        return ex; // Return the DioError or handle it accordingly
      }
    }
  }
